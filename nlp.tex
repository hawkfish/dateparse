...
%Also allows unsupervised learning of parsers from corpora.
...
(add notation for how rules are defined)

Given a CFG grammar $G$, we define  $\tau_G(s)$ as the set of all possible parse trees for input date-time string $s$.  For any $t \in \tau(G)$, the probability $p(t) \ge 1$. In addition, $\sum_{t \in \tau_{G}} = 1$. The parser then chooses the parse tree with the maximum probability, \textit{i.e.} given a date-time string $s$, as input, we determine the highest scoring parse tree $t$ as  $max_{t \in \tau(s)} p(t)$.
...
\subsection{Training the PCFG}

(used a subset of the public data)

Having defined initial probabilistic weights to the CFG, we train the PCFG with a known training set of date time formats to enhance the coverage of the weights to the various production rules of the grammar.

\subsection{Parsing the PCFG}
